...
must_haves:
  truths:
    - "Operative note template has all 17 sections including neurosurgery-specific ones (Implants/Hardware, Fluids/Drains, Neuromonitoring)"
  ...
  artifacts:
    - path: "src/skills/chart-skill/templates/operative-note.ts"
      provides: "17-section operative note template with neurosurgery-specific sections"
...
Export `operativeNoteTemplate: ChartTemplate` with `templateId: 'operative-note'`, `name: 'Operative Note'`, `version: '1.0.0'`, and exactly 17 sections (Surgeon and Assistant(s) are separate sections for template validation precision):
...
2. `buildVoiceInstructions(directives: VoiceDirectives): string` -- build a multi-line instruction string for the LLM:
   ...
   - Join all lines (directive lines + safety line) with newlines.

Import `VoiceDirectives` from `../types.js`.
  </action>
  <verify>npx vitest run test/unit/skills/chart-skill.test.ts --reporter=verbose</verify>
  <done>Three clinical note templates with correct section counts (17 operative, 14 H&P, 6 progress), voice adapter extracts and builds LLM instructions</done>
</task>
