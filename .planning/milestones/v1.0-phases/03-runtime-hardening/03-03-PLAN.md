...
5. `check()` with a tool in prohibited_actions returns `{ allowed: false, layer: 'tool-policy', reason containing 'prohibited_actions' }`
...
**Step 2: GREEN — Create canary stub then implement engine**

Before writing `engine.ts`, create a minimal `src/hardening/canary.ts` stub so that the engine's import resolves. This stub will be replaced with the full implementation in Task 2.
...
Now replace `src/hardening/engine.ts` entirely:
...
Replace the stub `src/hardening/canary.ts` (created in Task 1) with the full implementation:
