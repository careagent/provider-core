...
must_haves:
  truths:
    ...
    - "Full integration test verifies credential gating, integrity verification, and audit trail end-to-end"
  artifacts:
...
- Load skills
- Read AUDIT.log
- Verify entries in order: skill_discovery, skill_credential_check (allowed), skill_integrity_check (allowed), skill_load (allowed)
...
Clean up all temp directories in afterEach.
  ...
  <done>All integration tests pass verifying credential gating, integrity verification, audit logging, and CANS rules end-to-end</done>
</task>
...
- Integration tests verify all 7 SKIL requirements end-to-end
