...
describe('generateProposals', () => {
  ...
  it('creates multiple proposals from multiple divergences', () => {
    const divergences: DivergencePattern[] = [
      makeDivergence({ field_path: 'clinical_voice.tone', observation_count: 8 }),
      makeDivergence({
      ...
      makeDivergence({
        field_path: 'provider.institution',
        category: 'credential',
        observation_count: 5,
        declared_value: 'Old Hospital',
        most_common_observed: 'New Hospital',
      }),
    ];
    ...
    expect(proposals[2].field_path).toBe('provider.institution');
...
describe('generateDiffView', () => {
  it('produces correct diff format for string values', () => {
    ...
    expect(diff).toContain('@@ clinical_voice.tone @@');
  ...
  it('produces correct diff format for object values', () => {
    ...
    expect(diff).toContain('@@ provider.privileges @@');
  ...
  it('handles boolean values', () => {
    const divergence = makeDivergence({
      field_path: 'clinical_voice.eponyms',
      declared_value: true,
